        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (count($value) < 36) {
                        $is_valid =  false;
                        $error_msg = "Some of the columns are missing in row no. $row_no. Please, use latest CSV file template.";
                        $errors[$row_no][] = $error_msg;
                        // break;
                    }
                    ...
                    $image_url = $value[32];
                    if (!empty($image_url)) {
                        $name = explode('?',substr($image_url, strrpos($image_url, '/') + 1))[0];
                        $ext = substr($name, strrpos($name, '.') + 1);
                        if(!in_array($ext,['jpg','jpeg','png','gif','webp'])){
                            $name = $name.'.jpg';   
                        }
                        $image_url_name = time() . '_' . $name;
                        ...
                        $product_array['image'] = $image_url_name;
                    }

                    $product_array['product_description'] = isset($value[37]) ? $value[37] : null;

                    //Custom fields
                    if (isset($value[38])) {
                        $product_array['product_custom_field1'] = trim($value[38]);
                    } else {
                        $product_array['product_custom_field1'] = '';
                    }
                    if (isset($value[39])) {
                        $product_array['product_custom_field2'] = trim($value[39]);
                    } else {
                        $product_array['product_custom_field2'] = '';
                    }
                    if (isset($value[40])) {
                        $product_array['product_custom_field3'] = trim($value[40]);
                    } else {
                        $product_array['product_custom_field3'] = '';
                    }
                    if (isset($value[41])) {
                        $product_array['product_custom_field4'] = trim($value[41]);
                    } else {
                        $product_array['product_custom_field4'] = '';
                    }

                    //Add not for selling
                    $product_array['not_for_selling'] = !empty($value[42]) && $value[42] == 1 ? 1 : 0;

                    //Add enable stock
                    $enable_stock = trim($value[10]);
                    ...
                    //Add product type
                    $product_type = strtolower(trim($value[16]));
                    ...
                    //Add barcode type
                    $barcode_type = strtoupper(trim($value[9]));
                    ...
                    //Add tax type
                    $tax_type = strtolower(trim($value[15]));
                    ...
                    if ($product_array['enable_stock'] == 1) {
                        $product_array['alert_quantity'] = trim($value[11]);
                    }
                    ...
                    $sub_sub_category_name = trim($value[5]);
                    if (!empty($sub_sub_category_name)) {
                        ...
                        $product_array['sub_category_id_2'] = $sub_sub_category->id;
                    }

                    //Add Sub-Sub-Sub-Category
                    $sub_sub_sub_category_name = trim($value[6]);
                    if (!empty($sub_sub_sub_category_name)) {
                        $sub_sub_sub_category = Category::firstOrCreate(
                            ['business_id' => $business_id, 'name' => $sub_sub_sub_category_name, 'category_type' => 'product'],
                            ['created_by' => $user_id, 'parent_id' => $sub_sub_category->id]
                        );
                        $product_array['sub_category_id_3'] = $sub_sub_sub_category->id;
                    }

                    //Add Sub-Sub-Sub-Sub-Category
                    $sub_sub_sub_sub_category_name = trim($value[7]);
                    if (!empty($sub_sub_sub_sub_category_name)) {
                        $sub_sub_sub_sub_category = Category::firstOrCreate(
                            ['business_id' => $business_id, 'name' => $sub_sub_sub_sub_category_name, 'category_type' => 'product'],
                            ['created_by' => $user_id, 'parent_id' => $sub_sub_sub_category->id]
                        );
                        $product_array['sub_category_id_4'] = $sub_sub_sub_sub_category->id;
                    }
                    ...
                    if (!empty($sku)) {
                        ...
                        if(isset($product_array['type']) && $product_array['type'] == 'single'){
                    ...
                    //Add product expiry
                    $expiry_period = trim($value[12]);
                    $expiry_period_type = strtolower(trim($value[13]));
                    ...
                    //Weight
                    if (isset($value[27])) {
                        $product_array['weight'] = trim($value[27]);
                    } else {
                        $product_array['weight'] = '';
                    }

                    if(isset($product_array['type'])){
                        if ($product_array['type'] == 'single') {
                            //Calculate profit margin
                            $profit_margin = trim($value[21]);
                            ...
                            $dpp_inc_tax = trim($value[19]);
                            $dpp_exc_tax = trim($value[20]);
                            ...
                            $selling_price = !empty(trim($value[22])) ? trim($value[22]) : 0 ;
                            ...
                            //Opening stock
                            if (!empty($value[23]) && $enable_stock == 1) {
                                $product_array['opening_stock_details']['quantity'] = trim($value[23]);

                                if (!empty(trim($value[24]))) {
                                    $location_name = trim($value[24]);
                                ...
                                //Stock expiry date
                                if (!empty($value[25])) {
                                    $product_array['opening_stock_details']['exp_date'] = \Carbon::createFromFormat('m-d-Y', trim($value[25]))->format('Y-m-d');
                                } else {
                            ...
                            $variation_values_string = trim($value[18]);
                            ...
                            $dpp_inc_tax_string = trim($value[19]);
                            $dpp_exc_tax_string = trim($value[20]);
                            $selling_price_string = trim($value[22]);
                            $profit_margin_string = trim($value[21]);
                            ...
                            //Opening stock
                            if (!empty($value[23]) && $enable_stock == 1) {
                                $variation_os = array_map('trim', explode('|', $value[23]));
                                ...
                                foreach ($variation_os as $k => $v) {
                                    ...
                                    if (!empty($value[25])) {
                                        $product_array['variation']['variations'][$k]['opening_stock_exp_date'] = \Carbon::createFromFormat('m-d-Y', trim($value[25]))->format('Y-m-d');
                                    } else {
                                        $product_array['variation']['variations'][$k]['opening_stock_exp_date'] = null;
                                    }
                                }
                ...
                if (!empty($formated_data)) {
                    foreach ($formated_data as $index => $product_data) {
                        ...
                        if($product_data['type'] == 'variable'){
                            $sku_exist = Product::where('sku', $product_data['sku'])
                                ->where('business_id', $business_id)
                                ->first();
                        }
                        ...
                        if($sku_exist){
                            ...
                            if(count($variation_data['variations'])>0){
                                $variations_array = $variation_data['variations'];
                                foreach ($variation_data['variations'] as $key => $variation_item) {
                                    $variation_exist = Variation::leftJoin('product_variations','product_variations.id','variations.product_variation_id')
                                        ->where('variations.product_id',$product->id)
                                        ->whereRaw('LOWER(variations.name)="' . strtolower($variation_item['value']) . '"')
                                        ->where('product_variations.name',$variation_data['name'])
                                        ->first();
                                    if($variation_exist){
                                        unset($variations_array[$key]);
                                    }
                                }
                                $variation_data['variations'] = array_values($variations_array);
                            }

                            if(count($variation_data['variations'])>0){
                                //Create variable product variations
                                $variations = $this->productUtil->createVariableProductVariations(
                                    $product,
                                    [$variation_data],
                                    $business_id
                                );
                                ...
                                if(count($variations)>0){
                                    for($i=33; $i<37; $i++){
                                        if(array_key_exists($i,$additional_images[$index]) && count($additional_images[$index][$i])>0){
                                            foreach($additional_images[$index][$i] as $variantIndex => $image){
                                                if(property_exists($variations,$variantIndex)){
                                                    try {
                                                    ...
                                                    if($contents){
                                                        Storage::put('media/'.$image['name'], $contents);
                                                        $media = new \App\Media([
                                                        ...
                                                        $variations[$variantIndex]->media()->save($media);
                                                        $contents = null;
                                                    }
                                                }
                                            }
                            ...
                            $this->rackDetails(
                                ...
                                $imported_data[$index][29],
                                $imported_data[$index][30],
                                $business_id,
                                $product->id,
                                $index+1
                            );
                            ...
                            //Product locations
                            if (!empty($imported_data[$index][43])) {
                                $locations_array = explode(',', $imported_data[$index][43]);
                                $location_ids = [];
                            ...
                            // Base image url
                            if(!empty($base_image[$index])){
                                try {
                                    // $contents = file_get_contents($image_url);
                                    $ch = curl_init();
                                    curl_setopt($ch, CURLOPT_HEADER, 0);
                                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                    curl_setopt($ch, CURLOPT_URL, $base_image[$index]['image_url']);
                                    $contents = curl_exec($ch);
                                    curl_close($ch);
                                }catch(Exception $e) {
                                    $contents = null;
                                ...
                                if($contents){
                                    Storage::put('img/'.$base_image[$index]['name'], $contents);
                                    $contents = null;
                                }
                            }
                            ...
                            if ($product->type == 'single') {
                                $variation = $this->productUtil->createSingleProductVariation(
                                    $product,
                                    $product->sku,
                                ...
                                // Additional images
                                if(count($additional_images[$index])>0){
                                    foreach($additional_images[$index] as $image){
                                        try {
                                            // $contents = file_get_contents($image_url);
                                            $ch = curl_init();
                                            curl_setopt($ch, CURLOPT_HEADER, 0);
