                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if ($is_admin || auth()->user()->hasAnyPermission(['access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-truck', 'active' => request()->segment(2) == 'logs' ? false : request()->segment(1) == 'shipments']
                            );
                        }
                        
                        if ($is_admin || auth()->user()->hasAnyPermission(['access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                action('CourierlogController@index'),
                                __('lang_v1.courier_logs'),
                                ['icon' => 'fa fas fa-truck', 'active' => request()->segment(2) == 'logs']
                            );
                        }
            ...
            if (auth()->user()->can('business_settings.access')) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                                action('StorefrontController@index'),
                                __('business.settings'),
                                ['icon' => 'fa fas fa-cogs', 'active' => request()->segment(1) == 'storefront' && request()->segment(2) == 'settings', 'id' => "tour_step22"]
                            );
                            $sub->url(
                                action('StorefrontController@homepage'),
                                'Homepage',
                                ['icon' => 'fa fas fa-home', 'active' => request()->segment(1) == 'storefront' && request()->segment(2) == 'homepage', 'id' => "tour_step23"]
                            );
                        }
                    },
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        if (auth()->user()->can('business_settings.access')) {
                            ...
                            $sub->url(
                                action('CourierController@index'),
                                'Couriers',
                                ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'couriers']
                            );
                        }
