...
class ProductUtil extends Util
{
    ...
    public function createVariableProductVariations($product, $input_variations, $business_id = null)
    {
        ...
        foreach ($input_variations as $key => $value) {
            ...
            $product_variation = ProductVariation::firstOrCreate($product_variation_data);
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $this->woocommerceStockUpdate($before_status, $transaction, $data['product_id'], $data['variation_id'], $new_quantity, $purchase_line->quantity, $currency_details);
            } else {
                ...
                if ($transaction->status == 'received') {
                    ...
                    $this->woocommerceStockUpdate(null, $transaction, $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $currency_details);
    ...
    //woo commerce stock adjustment
    public function woocommerceStockUpdate($status_before, $transaction, $product_id, $variation_id, $new_quantity, $old_quantity, $currency_details)
    {
        try {
            $businessDetails = $this->getBusinessDetails($transaction->business_id);
            $woocommerceAppUrl = "";
            $woocommerceConsumerKey = "";
            $woocommerceConsumerSecret = "";
            if (!is_null($businessDetails)) {
                $woocommerceApiSettings = json_decode($businessDetails->woocommerce_api_settings);
                if (!is_null($woocommerceApiSettings)) {
            ...
            if ($status_before == 'received' && $transaction->status == 'received') {
                ...
                $qty_difference = $this->num_uf($new_quantity_f, null) - $this->num_uf($old_qty, null);
                if ($product->enable_stock == 1 && $qty_difference != 0) {
                    //call woo api
                    if (!empty($variation_id) || $variation_id != null) {
                        ...
                        if ($productvariationDetails->woocommerce_variation_id == null) {
                            $productdata = $woocommerce->get('products/' . $product->woocommerce_product_id);
                            //dd($productdata);
                        } else {
                            $productdata = $woocommerce->get('products/' . $product->woocommerce_product_id . '/variations/' . $productvariationDetails->woocommerce_variation_id);
                            //dd($productdata);
                        ...
                        $stockAdjust = $processedproductdata->stock_quantity + $qty_difference;
                        $data = [
                            'stock_quantity' => intval($stockAdjust)
                        ];
                        if ($productvariationDetails->woocommerce_variation_id == null) {
                            $updateProduct = $woocommerce->put('products/' . $product->woocommerce_product_id, $data);
                            //dd($updateProduct);

                        } else {
                            $updateProduct = $woocommerce->put('products/' . $product->woocommerce_product_id . '/variations/' . $productvariationDetails->woocommerce_variation_id, $data);
                            //dd($updateProduct);
                ...
                if ($product->enable_stock == 1) {
                    ...
                    if (!empty($variation_id) || $variation_id != null) {
                        ...
                        if ($processedproductdata->stock_quantity >= $qty_difference) {
                            $stockAdjust = $processedproductdata->stock_quantity - $qty_difference;
                            $data = [
                                'stock_quantity' => intval($stockAdjust)
                            ];
                ...
                $qty_difference = $new_quantity_f - 0;
                //dd($qty_difference);
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        if (!empty($search_term)) {
            ...
            if ($search_type == 'like') {
                ...
                //For search by brand (Arad)
                $brand_ids = [];
                if (in_array('brand', $search_fields)) {
                    $brands = DB::table('brands')->where('name', 'like', '%' . $search_term . '%')->get();
                    if ($brands) {
                ...
                $query->where(function ($query) use ($search_term, $search_fields, $brand_ids) {
                    ...
                    //For search by brand (Arad)
                    if (in_array('brand', $search_fields) && count($brand_ids) > 0) {
                        $query->orWhere(function ($q) use ($brand_ids) {
