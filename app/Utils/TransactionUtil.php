...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        try {
            ...
            $transaction = Transaction::create([
                ...
                'preferable_delivery_date' => $input['transaction_date'],
                //$input['preferable_delivery_date'],
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        if (!empty($input['preferable_delivery_date'])) {
            $update_date['preferable_delivery_date'] = $input['preferable_delivery_date'];
        }

        if (!empty($input['billing_address_id'])) {
            $update_date['billing_address_id'] = $input['billing_address_id'];
        }
        if (!empty($input['shipping_address_id'])) {
            $update_date['shipping_address_id'] = $input['shipping_address_id'];
        }
    ...
    ///New Function for woo order create 
    /**
     * Add line for payment
     *
     * @param object/int $transaction
     ...
     * @return boolean
     */
    public function createOrUpdatePaymentLinesForWoo($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true, $paymentMethod)
    {
        $payments_formatted = [];
        $edit_ids = [0];
        $account_transactions = [];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            if ($this->getTotalPaid($transaction->id) <= -1) {
                $paid_amount = 0;
            } else {
    ...
    public function getSellsCurrentFy($business_id, $start, $end)
    {
        ...
        // ->where('transactions.shipping_status', '!=', 'cancelled');
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            //->leftJoin('addresses', 'addresses.id', 'transactions.shipping_address_id')
            ->leftJoin('addresses as shippinad', function ($join) {
                $join->on('shippinad.id', '=', 'transactions.shipping_address_id')
                    ->orWhereNull('transactions.shipping_address_id');
            })
            ->leftJoin('addresses as billingad', function ($join) {
                $join->on('billingad.id', '=', 'transactions.billing_address_id')
                    ->orWhereNull('transactions.billing_address_id');
            })
            //->leftJoin('geo_unions', 'geo_unions.id', 'addresses.union_id')
            // ->leftJoin('geo_upazilas', 'geo_upazilas.id', 'geo_unions.geo_upazila_id')
            ->join(
            ...
            ->select(
                'transactions.id',
                'transactions.transaction_date',
                'transactions.preferable_delivery_date',
                'transactions.type',
                ...
                DB::raw('COALESCE(transactions.shipping_address, contacts.shipping_address) as shipping_address'),
                'shippinad.name as recipient_name',
                'shippinad.number as recipient_phone',
                'shippinad.address as recipient_address',
                // 'shippinad.default_zone as recipient_zone',
                'billingad.number as billing_number',

                DB::raw('(SELECT city_name FROM courier_zone_mappings WHERE id = shippinad.default_zone LIMIT 1) as recipient_city'),
                DB::raw('(SELECT zone_name FROM courier_zone_mappings WHERE id = shippinad.default_zone LIMIT 1) as recipient_zone'),

                // 'geo_upazilas.name_eng as recipient_city',
                // 'geo_unions.name_eng as recipient_zone',
                DB::raw('SUM(products.weight) as item_weight'),
                DB::raw("GROUP_CONCAT(products.name,'-',ROUND(tsl.quantity) separator ', ') as item_desc"),
    ...
    /**
     * common function to get
     * list sell for shipment
     * @param int $business_id
     *
     * @return object
     */
    public function getShipmentList($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            // ->leftJoin('transaction_payments as tp', 'transactions.id', '=', 'tp.transaction_id')
            ->leftJoin('transaction_sell_lines as tsl', function ($join) {
            ...
            // ->leftJoin('courier_zone_mappings as shippingmap', 'shippingmap.default_zone', 'shippinad.default_zone')
            // ->leftJoin('courier_zone_mappings as billingmap', 'billingmap.default_zone', 'billingad.default_zone')
            //->leftJoin('geo_unions', 'geo_unions.id', 'addresses.union_id')
            // ->leftJoin('geo_upazilas', 'geo_upazilas.id', 'geo_unions.geo_upazila_id')
            ->join(
                'business_locations AS bl',
            ...
            ->where('transactions.type', $sale_type)
            ->where('transactions.shipping_status', '!=', 'cancelled')
            ->where('transactions.shipping_status', '!=', 'shipped')
            ...
            ->select(
                ...
                DB::raw('(SELECT city_name FROM courier_zone_mappings WHERE id = shippinad.default_zone LIMIT 1) as recipient_city'),
                DB::raw('(SELECT zone_name FROM courier_zone_mappings WHERE id = shippinad.default_zone LIMIT 1) as recipient_zone'),
                // DB::raw('(SELECT area_name FROM courier_zone_mappings WHERE id = shippinad.default_zone LIMIT 1) as recipient_area'),

                // DB::raw('(SELECT city_name as recipient_city FROM courier_zone_mappings WHERE default_zone = shippinad.default_zone) LIMIT 1'),
                // 'geo_upazilas.name_eng as recipient_city',
                // 'geo_unions.name_eng as recipient_zone',
                DB::raw('SUM(products.weight) as item_weight'),
                DB::raw("GROUP_CONCAT(products.name,'-',ROUND(tsl.quantity) separator ', ') as item_desc"),
    ...
    //update woo commerce order 
    public function updateWooOrder($status, $woo_orderId, $business_id)
    {
        ...
        $woo_status = [
            'delivered' => 'completed',
            'cancelled' => 'cancelled',
            "Pickup_Requested" => 'on-hold',
            "Assigned_for_Pickup" => 'on-hold',
            "Picked" => 'on-hold',
            // "Pickup_Failed",
            // "Pickup_Cancelled",
            // "At_the_Sorting_HUB",
        ...
        if (in_array($status, array_keys($woo_status))) {
            $newStatus = $woo_status[$status];
            $data = ['status' => $newStatus];
            $updateProduct = $woocommerce->put('orders/' . $woo_orderId, $data);
